...
async def create_currency(
    ...
    stmt = select(Currency).where(Currency.code == currency_schema.code)
    result = await session.execute(stmt)
    ...
    if not currency_schema.is_crypto:

        try:
            currency_type = CurrencyType(currency_schema.code)
        except Exception:
            raise HTTPException(status_code=400, detail="Invalid ISO currency code")
